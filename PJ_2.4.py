...
class LearnedPositionalEncoding(nn.Module):
    """
    Drop-in replacement for sinusoidal positional encoding.
    Uses trainable position embeddings instead of fixed sin/cos values.
    """
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000) -> None:
        ...
        positions = torch.arange(0, max_len).unsqueeze(0)  # [1, max_len]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        seq_len = x.size(1)
        pos = self.positions[:, :seq_len].to(x.device)  # [1, seq_len]
...
def make_model(use_learned_positional: bool, vocab_de: Dict[str, int], vocab_en: Dict[str, int]):
    """
    Creates a fresh model.
    For learned positional embeddings, monkey-patch model.PositionalEncoding.
    """
    # Prevent auto-loading of any checkpoint from the model file.
    ...
    original_positional = model_module.PositionalEncoding

    if use_learned_positional:
        model_module.PositionalEncoding = LearnedPositionalEncoding
    ...
    # Restore class globals immediately after construction
    model_module.PositionalEncoding = original_positional
...
def train_one_run(
    ...
    """
    model = make_model(use_learned_positional, vocab_de, vocab_en).to(device)
    ...
    print(f"Positional encoding: {'LEARNED' if use_learned_positional else 'SINUSOIDAL'}")
    print(f"==================================================")
    ...
    summary_rows = [{
        ...
        "positional_encoding": "learned" if use_learned_positional else "sinusoidal",
    }]
...
def save_plots(epoch_sinusoidal: List[Dict], epoch_learned: List[Dict]) -> None:
    epochs = [row["epoch"] for row in epoch_sinusoidal]

    # Loss
    plt.figure(figsize=(10, 5))
    plt.plot(epochs, [r["train_loss"] for r in epoch_sinusoidal], label="Sinusoidal Train Loss")
    plt.plot(epochs, [r["train_loss"] for r in epoch_learned], label="Learned Train Loss")
    plt.plot(epochs, [r["val_loss"] for r in epoch_sinusoidal], label="Sinusoidal Val Loss")
    plt.plot(epochs, [r["val_loss"] for r in epoch_learned], label="Learned Val Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Positional Encoding Comparison: Loss Curves")
    plt.legend()
    plt.tight_layout()
    ...
    # Validation accuracy
    plt.figure(figsize=(10, 5))
    plt.plot(epochs, [r["val_accuracy"] for r in epoch_sinusoidal], label="Sinusoidal Val Accuracy")
    plt.plot(epochs, [r["val_accuracy"] for r in epoch_learned], label="Learned Val Accuracy")
    plt.xlabel("Epoch")
    plt.ylabel("Validation Accuracy")
    plt.title("Positional Encoding Comparison: Validation Accuracy")
    plt.legend()
    plt.tight_layout()
    ...
    # Validation BLEU
    plt.figure(figsize=(10, 5))
    plt.plot(epochs, [r["val_bleu"] for r in epoch_sinusoidal], label="Sinusoidal Val BLEU")
    plt.plot(epochs, [r["val_bleu"] for r in epoch_learned], label="Learned Val BLEU")
    plt.xlabel("Epoch")
    plt.ylabel("Validation BLEU")
    plt.title("Positional Encoding Comparison: Validation BLEU")
    plt.legend()
    plt.tight_layout()
...
def main() -> None:
    ...
    # Run 1: Sinusoidal positional encoding
    sinusoidal_epochs, sinusoidal_sample, sinusoidal_summary = train_one_run(
        run_name="sinusoidal_positional",
    ...
    # Run 2: Learned positional encoding
    learned_epochs, learned_sample, learned_summary = train_one_run(
        run_name="learned_positional",
    ...
    # Save per-run CSVs
    save_csv(
        os.path.join(CONFIG["output_dir"], "sinusoidal_epoch_metrics.csv"),
        sinusoidal_epochs,
        fieldnames=["run_name", "epoch", "train_loss", "val_loss", "val_accuracy", "val_bleu", "learning_rate"],
    )
    save_csv(
        os.path.join(CONFIG["output_dir"], "learned_epoch_metrics.csv"),
        learned_epochs,
        fieldnames=["run_name", "epoch", "train_loss", "val_loss", "val_accuracy", "val_bleu", "learning_rate"],
    )
    ...
    # Save combined CSV
    combined_rows = []
    for row in sinusoidal_epochs:
        combined_rows.append({"setting": "sinusoidal", **row})
    for row in learned_epochs:
        combined_rows.append({"setting": "learned", **row})

    save_csv(
        os.path.join(CONFIG["output_dir"], "combined_epoch_metrics.csv"),
        combined_rows,
        fieldnames=["setting", "run_name", "epoch", "train_loss", "val_loss", "val_accuracy", "val_bleu", "learning_rate"],
    )

    # Save sample translation CSVs
    save_csv(
        os.path.join(CONFIG["output_dir"], "sinusoidal_sample_translation.csv"),
        sinusoidal_sample,
        fieldnames=["run_name", "source_sentence", "reference_translation", "predicted_translation"],
    )
    save_csv(
        os.path.join(CONFIG["output_dir"], "learned_sample_translation.csv"),
        learned_sample,
        fieldnames=["run_name", "source_sentence", "reference_translation", "predicted_translation"],
    )

    # Save summary CSV
    summary_rows = sinusoidal_summary + learned_summary
    save_csv(
        os.path.join(CONFIG["output_dir"], "final_summary.csv"),
        summary_rows,
        fieldnames=["run_name", "final_train_loss", "final_val_loss", "final_val_accuracy", "final_val_bleu", "positional_encoding"],
    )

    # Comparison plots
    save_plots(sinusoidal_epochs, learned_epochs)

    # Small summary print
    print("\nDone.")
    print(f"All CSV and PNG files are saved in: {CONFIG['output_dir']}")
    print("Important files created:")
    print(f"- {os.path.join(CONFIG['output_dir'], 'sinusoidal_epoch_metrics.csv')}")
    print(f"- {os.path.join(CONFIG['output_dir'], 'learned_epoch_metrics.csv')}")
